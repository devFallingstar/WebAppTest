		...
		if (D != null && D.size() != 0) {
			for (Node node : D) {
				...
				// If the node is a relationship type,
					...
					// A variable to get the key values ​​of the entity associated with the
					// relationship
					if (node.isTemporal) {
					...
					/*
					 * for(Node tmpNode:getAttributes(node)) { if(isAttribute(tmpNode)) { Attribute
					 * attNode = (Attribute)tmpNode; boolean tmp = false;
					 * if(!attNode.isTemporal&&attNode.getIsMulti()) {
					 * 
					 * Relation relation5=new Relation(); if(!tmp){
					 * relation5.setName(node.name+"_"+attNode.name); tmp=true; }
					 * relation5.addKeyLists(attNode.name);
					 * 
					 * if(attNode.getTypeOfMulti().equals("1")) {
					 * relation5.setAttLists(relationshipKeys); for(Node
					 * tmpNode2:getAttributes(node)) { if(isAttribute(tmpNode2)) { Attribute
					 * attNode2 = (Attribute)tmpNode2; if(attNode2.getIsKey()){
					 * relation5.addAttLists(attNode2.name); } } }
					 * 
					 * } else { for(String tmp2:relationshipKeys) { relation5.addKeyLists(tmp2); }
					 * 
					 * for(Node tmpNode2:getAttributes(node)) { if(isAttribute(tmpNode2)) {
					 * Attribute attNode2 = (Attribute)tmpNode2; if(attNode2.getIsKey()){
					 * relation5.addKeyLists(attNode2.name); } } } } R.add(relation5); } } }
					 */
